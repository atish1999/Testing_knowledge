...
public class Rope {
	...
	static int n,k,a[];
//	we have to cut k pieces
	static boolean good(double x) { // if we can cut k pieces of size x or not
		long s=0;
		for(int i=0; i<n; i++) {
			s+=(a[i]/x);// there will be {a[i]/x} pieces of size x each in length a[i]
		}
		return s>=k;
	}
	public static void main(String[] args) throws java.lang.Exception {
		...
		while (t-- > 0) {
			...
			double l=0;  // l is for good size
			double r=1e8;// r is for bad size(because max of a[i]=10^7)
// we have to find the maximum length for which size will be good.
			for(int i=0; i<100; i++) {
				double m=(l+(r-l)/2);
				if(good(m)) l=m;
				else r=m;
