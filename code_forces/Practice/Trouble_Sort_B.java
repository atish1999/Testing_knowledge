...
public class Trouble_Sort_B implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Trouble_Sort_B(), "Main", 1 << 26).start();
	}
	...
	public void run() {
		...
		while (t-- > 0) {
			int n = fr.nextInt();
			int val[]=new int[n],typ[]=new int[n];
			int c=0;
			for(int i=0; i<n; i++) {
				val[i]=fr.nextInt();
				if(i!=0 && val[i]>=val[i-1]) ++c;
			}
			int c0=0,c1=0;
			for(int i=0; i<n; i++) {
				typ[i]=fr.nextInt();
...
//			If there is at least 1 c0 and at least 1 c1 then answer can be possible
			System.out.println((c==n-1 || (c0>=1 && c1>=1))?"Yes":"No");
		}
