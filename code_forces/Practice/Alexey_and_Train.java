...
/*
 *   always remeber to check undeflow and overflow conditions
 */
public class Alexey_and_Train implements Runnable {
...
//	static final FastReader fr = new FastReader("/home/atish/eclipse-workspace/Codeforces/src/contest/A/input.txt");

	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Alexey_and_Train(), "Main", 1 << 26).start();
	}
	...
	void solve() {
		...
		Pair a[] = new Pair[n];
		for (int i = 0; i < n; i++) {
			a[i] = new Pair(fr.nextInt(), fr.nextInt());
		}
		int t[] = fr.nextIntArray(n);
		long time = t[0] + a[0].x;
		if (n == 1) {
			out.print(time);
			return;
		}
		time += (CP.ceil_div(a[0].y - a[0].x, 2));
		time = Math.max(time, a[0].y);
		for (int i = 1; i < n; i++) {
			time += (a[i].x - a[i - 1].y + t[i]);
			if (i == n - 1)
				break;
			time += (CP.ceil_div(a[i].y - a[i].x, 2));
			time = Math.max(time, a[i].y);
		}
		out.print(time);
	}
