...
public class Regular_Bracket_101_edu implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Regular_Bracket_101_edu(), "Main", 1 << 26).start();
	}
	...
	public void run() {
		...
		int t = fr.nextInt();
		while (t-- > 0) {
			s=fr.nextLine().toCharArray();
			int op=0,cl=0,n=s.length,q=0;
			for(int  i=0; i<n; i++) {
				if(s[i]=='(') op=i;
				else if(s[i]==')') cl=i;
				else ++q;
			}
			if(op==(n-1) || cl==0 || ((q&1)==1)) {
