...
public class String_LCM_102_edu implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new String_LCM_102_edu(), "Main", 1 << 26).start();
	}
	...
	public void run() {
		...
		while (t-- > 0) {
			String f=fr.next(),s=fr.next();
			if(f.length()<s.length()) {
				String temp=f;
				f=s;
				s=temp;
			}
			int n1=f.length(),n2=s.length();
			int lcm=(n1*n2)/CP.i_gcd(n1, n2);
			if(n1%n2!=0) {
				f=f.repeat(lcm/n1);
			}
			boolean ok=true;
			for(int i=0; i<=f.length()-n2; i+=n2) {
				if(!f.substring(i, n2+i).equals(s)) {
					ok=false;
					break;
				}
			}
