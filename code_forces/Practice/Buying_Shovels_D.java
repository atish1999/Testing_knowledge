...
public class Buying_Shovels_D implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Buying_Shovels_D(), "Main", 1 << 26).start();
	}
...
//			we have to find the largest factor of n which is not greater than k
			if(k>=n) {
				System.out.println(1);
			}else {
				if(CP.isPrime(n) || k==1) System.out.println(n);
				else {
					long ans=n;
					for(int i=1; i*i<=n; i++) {
						if(n%i==0) {
...
//					(n/ans) will give the largest factor not greater than k
