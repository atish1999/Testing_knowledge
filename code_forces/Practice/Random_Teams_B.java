...
public class Random_Teams_B implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Random_Teams_B(), "Main", 1 << 26).start();
	}
	...
	public void run() {
		...
		while (t-- > 0) {
			...
			if(n%m==0) {
		        long x=fun((n/m)-1),min=x*m;
		        long y=n-(m-1),max=fun(y-1);
		        System.out.println(min+" "+max);
			}else {
				long rem=n%m,min=rem*fun(n/m)+(m-rem)*fun(n/m-1);
				long y=n-(m-1),max=fun(y-1);
				System.out.println(min+" "+max);
			}
		}
