...
public class Two_buttons_Method_2 implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Two_buttons_Method_2(), "Main", 1 << 26).start();
	}
//	iterative_way
	static void it(int n, int m) {
		if(n>=m) {
			System.out.println(n-m);
			return;
		}
//		it is easier to convert m into n by using exact reverse operations

		int cnt=0;
		while(m>n) {
			if((m&1)==0) { // when even m->(m/2)
				m/=2;
			}else {// when odd convert it to even and doing the above
				++m;//we can't subtract -1 beacuse we are doing the reverse operations 
//				to convert m to n
			}
			++cnt;
		}
...
//	recursive_way
	static int re(int n, int m) {
		if(n>=m) return(n-m);
		if((m&1)==0) {
			return 1+re(n,m/2);
		}else {
			return 2+re(n,(m+1)/2); // here we are doing two steps 
//			first we are converting odd m-> even m by adding 1
//			then we are dividing 
		}
	}
	@Override
	public void run() {
