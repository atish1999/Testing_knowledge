...
public class Farewell_or_Best_Wishes implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Farewell_or_Best_Wishes(), "Main", 1 << 26).start();
	}
	...
	public void run() {
		...
		int t = fr.nextInt();
		while (t-- > 0) {
			long n=fr.nextLong(),m=fr.nextLong(),x=fr.nextLong(),y=fr.nextLong();
			if(x==1 || y==m) {
				System.out.println("BestWishes");
				continue;
			}
			long a=(y-1);
			long no=(x-1),not=2*(n-1);
			if((a-no)>=0 && (a-no)%not==0) {
				System.out.println("BestWishes");
				continue;
			}
			no=(n-x)+(n-1);
