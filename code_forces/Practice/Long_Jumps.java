...
public class Long_Jumps implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Long_Jumps(), "Main", 1 << 26).start();
	}
	static long sum;
	static void dfs(int s,int n,int arr[]) {
		vis[s]=true;
		sum+=arr[s-1];
		for(Pair  child: adj[s]) {
			int a=child.x;
			if(a<=n ) {
				if(!vis[a]) {
					dfs(a,n,arr);
	...
	public void run() {
		...
		while (t-- > 0) {
			...
			for(int i=0; i<n; i++) {
				a[i]=fr.nextInt();
				adj[i+1].add(new Pair(i+1+a[i], a[i]));
