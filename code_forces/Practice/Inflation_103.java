...
public class Inflation_103 implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Inflation_103(), "Main", 1 << 26).start();
	}
	...
	public void run() {
		...
		while (t-- > 0) {
			...
			long a[] = fr.nextLongArray(n);
			long total=a[0];
			long max=0;
			for(int i=1; i<n; i++) {
				long r=100*a[i];
				if(total*k<r) {
					 max=Math.max(max,(r+k-1)/k - total);
				}
				total+=a[i];
			}
