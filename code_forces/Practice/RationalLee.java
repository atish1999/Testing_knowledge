...
public class RationalLee implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new RationalLee(), "Main", 1 << 26).start();
	}
	...
	public void run() {
		...
		while (t-- > 0) {
			...
			int f[]=fr.nextIntArray(k);
			CP.sort(a);
			Arrays.sort(f);
			int store[]=new int[k];
			long max=0;
			for(int i=0; i<k; i++) {
				max+=a[i];//storing the max
				store[i]=a[i];
				--f[i];
			}
			int c=1;
			for(int i=k-1; i>=0; i--) {//storing the minimum
				if(f[i]==0) {
					max+=store[i];
				}else {
					max+=a[n-c];	
