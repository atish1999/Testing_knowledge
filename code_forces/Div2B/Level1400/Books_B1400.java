...
public class Books_B1400 {
	...
	public static void main(String[] args) throws java.lang.Exception {
		...
		while (t-- > 0) {
			...
			int s=0,ans=0,sum=0,c=0;
			for(int i=0; i<n; ) {
				if(sum+a[i]<=f) {// if current sum is less than f then we can add the
					sum+=a[i];// element into sum and increase our counter by 1
					++c;
					++i;
				}else {
					sum-=a[s];// otherwise we should deduct the number from starting
					++s;// and also increase the increasing pointer and decrease the
					--c;// total count by 1
				}
				ans=Math.max(ans, c);
			}
