...
public class ONe_Zero_Swap_M2 implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new ONe_Zero_Swap_M2(), "Main", 1 << 26).start();
	}
	...
	public void run() {
		...
		while (t-- > 0) {
			int n = fr.nextInt();
			char[] s=fr.nextLine().toCharArray(),p=fr.nextLine().toCharArray();
			int count=0;
			for(int i=0; i<n; i++) {
				if(s[i]==p[i]) continue;
				else if(s[i]=='1') ++count;
