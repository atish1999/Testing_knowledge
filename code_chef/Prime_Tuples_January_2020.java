...
public class Prime_Tuples_January_2020 implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Prime_Tuples_January_2020(), "Main", 1 << 26).start();
	}
	...
	public void run() {
		...
		List<Integer> primes=CP.sieve((int)(1e6+1));
		int prefix[]=new int[(int)(1e6+1)];
		for(int i=1; i<primes.size(); i++) {
			if(primes.get(i)-2==primes.get(i-1)) {
				prefix[primes.get(i)]=1;
			}		
		}
		...
		while (t-- > 0) {
			long n=fr.nextLong();
			System.out.println(prefix[(int) n]);
		}
	...
	static class CP {
		...
		static int lower_Bound(List<Integer> prime, long n) {//closest to the right
